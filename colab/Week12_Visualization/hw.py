...
st.sidebar.slider(
    "Select Range of Data Points:", min_value=0, max_value=6, value=(0, 6), step=1
)
...
)  # Optional: hide histograms on diagonals
...
st.line_chart(inertias, use_container_width=True)
...
with col1:
    ...
    fig_cluster = px.scatter(
        ...
        color_discrete_map={"0": "#4470AD", "1": "#CCDBEE", "2": "#FF4B4B"},
        labels={"Cluster": "Cluster"},
    )
    fig_cluster.update_traces(marker=dict(size=10))
    fig_cluster.update_layout(
        xaxis_title=feature_names[2], yaxis_title=feature_names[3], showlegend=True
    )
