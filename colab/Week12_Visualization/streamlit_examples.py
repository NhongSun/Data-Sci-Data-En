# streamlit run colab/Week12_Visualization/streamlit_examples.py
...
def charts():
    ...
    chart_data = pd.DataFrame(
        np.random.randn(20, 3), columns=["Line 1", "Line 2", "Line 3"]
    )
    ...
    fig_scatter = px.scatter(
        data, x="Sales", y="Revenue", color="Category", title="Sales vs Revenue"
    )
    st.plotly_chart(fig_scatter, use_container_width=True)

    # Plotly Time Series
    fig_time = px.line(
        data, x="Date", y="Sales", color="Category", title="Time Series Analysis"
    )
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=data["Date"], y=data["Sales"], name="Sales", mode="lines")
    )
    fig.add_trace(
        go.Scatter(
            x=data["Date"], y=data["Revenue"], name="Revenue", mode="lines", yaxis="y2"
        )
    )
...
# Main App Structure
def main():
    st.set_page_config(page_title="Streamlit Examples", layout="wide")

    st.sidebar.title("Navigation")
    page = st.sidebar.selectbox(
        "Choose an example",
        ["Text Elements", "Data Display", "Charts", "Input Widgets", "Layout"],
    )
