...
@st.cache_data
def get_all_data():
    root = "colab/Week12_Visualization/streamlit_kmeans/Datasets/"
...
def analyze_dataset_statistics(data):
    ...
    avg_length_positive = np.mean(
        [length for length, label in zip(text_lengths, labels) if label == 1]
    )
    avg_length_negative = np.mean(
        [length for length, label in zip(text_lengths, labels) if label == 0]
    )
...
def create_visualizations(training_data, evaluation_data, vectorizer, classifier):
    ...
    with col1:
        ...
        st.metric(
            "Average Words in Positive Reviews", f"{stats['avg_length_positive']:.1f}"
        )

    with col2:
        st.metric(
            "Positive/Negative Ratio", f"{stats['positive']/stats['negative']:.2f}"
        )
        st.metric(
            "Average Words in Negative Reviews", f"{stats['avg_length_negative']:.1f}"
        )
    ...
    with col1:
        ...
        fig_pos_words = go.Figure(
            data=[
                go.Bar(
                    x=list(pos_words.keys()),
    ...
    with col2:
        ...
        fig_neg_words = go.Figure(
            data=[
                go.Bar(
                    x=list(neg_words.keys()),
...
def print_result(result):
    ...
    sentiment = (
        "Positive" if analysis_result == 1 else "Negative"
...
review = st.text_area(
    "Enter Text for Analysis", "Write your review here...", height=100
)
