...
def load_network_from_file(uploaded_file, file_format):
    ...
    try:
        if file_format == "CSV":
            ...
            if len(df.columns) < 2:
                raise ValueError(
                    "CSV file must have at least two columns for source and target nodes"
                )
...
class NetworkVisualizer:
    ...
    def create_interactive_network(
        ...
        try:
            ...
            centrality = nx.degree_centrality(self.G)
            st.warning(
                f"Failed to compute {centrality_metric} centrality, using degree centrality instead."
            )
        ...
        if max_cent > min_cent:
            size_scale = lambda x: min_size + (x - min_cent) * (max_size - min_size) / (
                max_cent - min_cent
            )
        else:
        ...
        if communities:
            ...
            color_map = {
                com: self.colors[i % len(self.colors)]
        ...
        with st.sidebar:
            ...
            if data_source == "Sample Networks":
                ...
                file_format = st.selectbox(
                    "Choose file format", ["CSV", "GML", "GraphML"]
                )
    ...
    with tab_analysis:
        if G is not None:
            ...
            with right_col:
                ...
                st.dataframe(
                    centrality_df, height=400  # Fixed height to match the plot
                )
        else:
